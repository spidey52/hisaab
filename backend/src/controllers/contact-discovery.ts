...
import { contactDiscoveryUsage, users, withTransaction } from "../db";
import {
...
export async function discover(c: Context) {
  ...
  const phones = normalizeDiscoveryPhones(payload.phones);
  if (phones.length === 0) {
  ...
  const matches = await withTransaction(async (client) => {
    ...
    const usage = await client
      .select({
        request_count: sql<
          number
        >`coalesce(${contactDiscoveryUsage.requestCount}, 0)::int`.mapWith(
          Number,
        ),
        phone_count: sql<
          number
        >`coalesce(${contactDiscoveryUsage.phoneCount}, 0)::int`.mapWith(
          Number,
        ),
        retry_after_seconds: sql<
          number
        >`greatest(1, extract(epoch from (date_trunc('hour', current_timestamp) + interval '1 hour' - current_timestamp))::int)`
          .mapWith(
            Number,
          ),
      })
    ...
    if (
      ...
      throwApiError(
        429,
        "Contact discovery is temporarily limited. Try again later.",
        {
          code: "RATE_LIMITED",
          headers: {
    ...
    await client
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          phoneCount:
            sql`${contactDiscoveryUsage.phoneCount} + excluded.phone_count`,
        },
      });
