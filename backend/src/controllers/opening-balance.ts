...
export async function upsertOpeningBalance(c: Context) {
  ...
  const result = await withTransaction(async (client) => {
    ...
    const partyRows = await client
      ...
      .for("update");
    if (!partyRows[0]) {
      throwApiError(404, "Customer or supplier not found.");
    }
    ...
    if (
      ...
      throwApiError(409, "The opening balance changed on another device.", {
    ...
    const now = nowDate();
    const effect = normalized.direction === "receive"
...
function openingMatches(
  ...
  const effect = expected.direction === "receive"
