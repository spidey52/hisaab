...
import { formatLedgerDate, isValidDateOnly } from "@/lib/date-utils";
import {
  entryAmountSign,
  entryDirectionLabel,
  entryDirectionTone,
  ...
  const visibleParties = useMemo(() => {
    ...
    return data.parties.filter((party) => {
      ...
      if (filter === "recent")
        matchesFilter =
          !archived &&
          now - new Date(party.createdAt).getTime() < 30 * 24 * 60 * 60 * 1000;
      ...
      <main className="app-main">
        <header className="app-header">
          <div>
            ...
            <span>{formatLongDate(new Date(), data.company.timezone)}</span>
          </div>
    ...
    <div className="party-statement">
      ...
      <div className="statement-list printable-statement">
        <div className="print-heading">
          ...
          <p>
            {dateRangeLabel} · Generated {formatLongDate(new Date(), timezone)}
          </p>
        </div>
      ...
      <div className="entry-detail">
        ...
        <dl>
          ...
          {entry.editedAt ? (
            <Detail
              label="Edited"
              value={`${new Date(entry.editedAt).toLocaleString("en-IN")} · previous version kept`}
            />
          ) : null}
        </dl>
        {entry.status === "cancelled" ? (
    ...
    <div className="app-view more-view">
      ...
      <section className="settings-section">
        ...
        <div className="settings-actions">
          <a className="setting-action" href="/api/export?format=json"><Download /><span><strong>Export all data</strong><small>Download a complete JSON backup</small></span><ChevronRight /></a>
          <a className="setting-action" href="/api/export?format=csv"><FileText /><span><strong>Export entries as CSV</strong><small>Open in spreadsheet software</small></span><ChevronRight /></a>
          <button
                ...
                >
                  ...
                  {item.retryAfter ? (
                    <small className="queue-retry-time">
                      ...
                      {new Date(item.retryAfter).toLocaleString("en-IN")}
                    </small>
                  ) : null}
                  {editing ? (
